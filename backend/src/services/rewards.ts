...
const REWARD_TIER = 500_000        // COP acumulados para activar recompensa
...
export async function checkAndEmitReward(clientId: string, completedOrderId: string): Promise<void> {
  ...
  // 2. ¿Cruzó el umbral?
  if (totalSpend < REWARD_TIER) return
  ...
  if (existing) return // Ya fue recompensado por este umbral
  ...
  const { data: rewardCode, error: insertError } = await supabaseAdmin
    .from('discount_codes')
    .insert({
      code,
  ...
  if (insertError) {
    if (insertError.code === '23505') return // Unique constraint violation
