...
router.get('/summary', async (req, res) => {
    ...
    let ordersQuery = supabase_1.supabaseAdmin
        .from('orders')
    ...
    let fromIdx = 0;
    while (true) {
        const { data: page, error: pageErr } = await supabase_1.supabaseAdmin
            .from('products')
            .select('stock, price')
            .eq('is_active', true)
    ...
    const { count: draftCount } = await supabase_1.supabaseAdmin
        .from('orders')
        .select('*', { count: 'exact', head: true })
        .eq('status', 'draft')
        .is('deleted_at', null);
...
// ============================================================
router.get('/payment-breakdown', async (_req, res) => {
    const { data, error } = await supabase_1.supabaseAdmin
        .from('orders')
